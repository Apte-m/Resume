...
Функциональность: CRUD -> api -> post
  ...
  Структура сценария:  получения записей
    ...
    Дано ендпоинт "запись" получаем и проверяем и "<статус код>"
  ...
  Структура сценария:  создание записи
    ...
    Дано ендпоинт "запись" и создаем запись и "<статус код>"
      | title  | <title>  |
      | body   | <body>   |
      | userId | <userId> |
  ...
  Структура сценария:  редактирование записи
    ...
    Дано ендпоинт "запись" и редактируем запись и "<статус код>"
      | title  | <title>  |
      | body   | <body>   |
      | userId | <userId> |
  ...
  Сценарий:  удаление записи
    ...
    Затем  на едпоинте "запись" удалем запись "1" и код "200"